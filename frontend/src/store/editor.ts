...
function buildGeometryFromObject(o: SceneObject): THREE.BufferGeometry {
  switch (o.geometry) {
    ...
    case 'sphere': {
      ...
      return new THREE.SphereGeometry(p?.radius ?? 0.5, 32, 16)
    }
    case 'cylinder': {
    ...
    case 'torus': {
      ...
      return new THREE.TorusGeometry(p?.radius ?? 0.5, p?.tube ?? 0.2, 16, 64)
    }
    case 'plane': {
...
function createObject<K extends GeometryKind>(kind: K, name?: string, params?: GeometryParamsMap[K]): SceneObject {
  return {
    id: nanoid(8), 
...
const initialState: EditorState & { isTransforming?: boolean; isGizmoInteracting?: boolean } = {
  objects: [],
  selectedId: null,
  mode: 'translate',
  editorMode: 'object',
...
export const useEditor = create<EditorStore>()((set) => ({
  ...initialState,
  showChatPanel: true,
  addObject: (kind, params) => set((state) => {
  ...
  deleteSelected: () => set((state) => {
    ...
    const next = produce(state, (draft) => {
      draft.past.push(snapshot(state))
      draft.future = []
      draft.objects = draft.objects.filter(o => o.id !== state.selectedId)
  ...
  setShowChatPanel: (visible) => set((state) => ({ ...state, showChatPanel: visible })),
  toggleChatPanel: () => set((state) => ({ ...state, showChatPanel: !state.showChatPanel })),
  ...
  endTransform: () => set((state) => produce(state, (draft) => { draft.isTransforming = false })),
  updateTransform: (id, partial) => set((state) => {
    const next = produce(state, (draft) => {
      const obj = draft.objects.find((o: SceneObject) => o.id === id)
      if (!obj) return
      if (partial.position) obj.position = { ...obj.position, ...partial.position }
      if (partial.rotation) obj.rotation = { ...obj.rotation, ...partial.rotation }
      if (partial.scale) obj.scale = { ...obj.scale, ...partial.scale }
    })
    return next
  }),
  updateMaterial: (id, partial) => set((state) => produce(state, (draft) => {
    const obj = draft.objects.find(o => o.id === id)
    if (obj) obj.material = { ...obj.material, ...partial }
  })),
  updateGeometry: <K extends GeometryKind>(id: string, kind: K, params?: GeometryParamsMap[K]) => set((state) => produce(state, (draft) => {
    const obj = draft.objects.find(o => o.id === id)
    if (!obj) return
    obj.geometry = kind
    obj.geometryParams = params as GeometryParamsMap[keyof GeometryParamsMap]
  })),
  toggleSnap: (enabled) => set((state) => ({ ...state, snap: { ...state.snap, enableSnapping: enabled } })),
  setSnap: (partial) => set((state) => ({ ...state, snap: { ...state.snap, ...partial } })),
  booleanOp: (op, aId, bId) => set((state) => {
    ...
    const resultGeom = (result.geometry as THREE.BufferGeometry).clone()
  ...
  addCheckpoint: (meta) => set((state) => produce(state, (draft) => {
    const id = nanoid(6)
    const label = meta.label ?? (meta.prompt ? meta.prompt.slice(0, 40) : 'Checkpoint')
    draft.checkpoints.unshift({ id, label, timestamp: Date.now(), prompt: meta.prompt, response: meta.response, state: snapshot(state) })
  })),
  restoreCheckpoint: (id) => set((state) => produce(state, (draft) => {
  ...
  deleteCheckpoint: (id) => set((state) => produce(state, (draft) => {
    draft.checkpoints = draft.checkpoints.filter(c => c.id !== id)
  })),
  undo: () => set((state) => {
