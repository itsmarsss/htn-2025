...
function buildGeometryFromObject(o: SceneObject): THREE.BufferGeometry {
  switch (o.geometry) {
    case 'box': return new THREE.BoxGeometry((o.geometryParams as any)?.width ?? 1, (o.geometryParams as any)?.height ?? 1, (o.geometryParams as any)?.depth ?? 1)
    case 'sphere': return new THREE.SphereGeometry((o.geometryParams as any)?.radius ?? 0.5, 32, 16)
    case 'cylinder': return new THREE.CylinderGeometry((o.geometryParams as any)?.radiusTop ?? 0.5, (o.geometryParams as any)?.radiusBottom ?? 0.5, (o.geometryParams as any)?.height ?? 1, 32)
    case 'cone': return new THREE.ConeGeometry((o.geometryParams as any)?.radius ?? 0.5, (o.geometryParams as any)?.height ?? 1, 32)
    case 'torus': return new THREE.TorusGeometry((o.geometryParams as any)?.radius ?? 0.5, (o.geometryParams as any)?.tube ?? 0.2, 16, 64)
    case 'plane': return new THREE.PlaneGeometry((o.geometryParams as any)?.width ?? 1, (o.geometryParams as any)?.height ?? 1)
    case 'custom': {
...
function createObject(kind: GeometryKind, name?: string, params?: any): SceneObject {
  return {
    id: nanoid(8),
...
export const useEditor = create<EditorStore>()((set, get) => ({
  ...
  endTransform: () => set((state) => produce(state, (draft: any) => { draft.isTransforming = false })),
  ...
  updateGeometry: (id, kind, params) => set((state) => produce(state, (draft) => {
    const obj = draft.objects.find(o => o.id === id)
    if (!obj) return
    obj.geometry = kind
